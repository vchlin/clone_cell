...
//!
//! In Rust, to enable interiorly mutating methods on a `struct` stored in an [`Rc`] without the
//! overhead of a [`RefCell`], we can wrap each of the fields in a [`std::cell::Cell`]. But its
//! [`get`] method is only implemented for types that are `Copy`. This is because if the `clone`
//! method obtains a reference to the `Cell`'s interior, it may be able to mutate its state. This
//! can cause undefined behavior, as demonstrated in this [example].
//!
...
//! 2. Use the provided proc macro to derive `PureClone` for user types, which ensures:
